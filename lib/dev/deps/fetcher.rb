  ...
  module Deps
    ...
    module Fetcher
      class << self
        ...
        def fetch_missing(root:, build_dir: "build", verbose: false)
        ...
        def fetch_git(repo, sha, dest_dir, quiet: false)
        ...
        def fetch_tarball(url, dest_dir, quiet: false)
          FileUtils.mkdir_p(File.dirname(dest_dir))
          FileUtils.rm_rf(dest_dir) if File.exist?(dest_dir)
          sha256_hex = nil
        ...
        def update_lockfile_hash(lock_path, name, sha256_hex)
