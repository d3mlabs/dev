...
class Dev::Builtins::ResetContainerCommandTest < Minitest::Test
  ...
  test "call removes the persistent container and reports what went" do
    ...
    BuildContainer.stubs(:image_with_tag).returns("myregistry/myapp-linux:content-abc123")
    BuildContainer.stubs(:reset_service!).returns(["dev-myapp-linux-content-abc123"])
  ...
  test "call reports when there is nothing to remove" do
    ...
    context = build_context(config)
    BuildContainer.stubs(:image_with_tag).returns("myregistry/myapp-linux:content-abc123")
    BuildContainer.stubs(:reset_service!).returns([])
    command = Dev::Builtins::ResetContainerCommand.new
    old_stdout = $stdout
    $stdout = StringIO.new
